...
class Row extends React.Component {
  render() {
    const { className, team, status, functions = [], name, sources } = this.props;
    ...
    return (
      ...
      <div className={css.join(' ')}>
        ...
        <DataColumn id="functions">{Array.isArray(functions) && functions.length > 0 ? functions.join(', ') : '-'}</DataColumn>
        <DataColumn id="sources">{Array.isArray(sources) && sources.length > 0 ? sources.join(', ') : '-'}</DataColumn>
